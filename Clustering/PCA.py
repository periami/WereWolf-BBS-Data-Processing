...
label_name = ["co", "div", "med", "prt", "dis_co", "else"]

# images = []
# for label, name in enumerate(label_name):
#     for angle in tqdm.tqdm(range(1)):
#         fig = plt.figure(figsize = (32, 32))
#         ax = fig.add_subplot(projection='3d')
#         for i in df['cluster'].unique():
#             tmp = pca_df.loc[pca_df['cluster'] == label]
#             ax.scatter(tmp[0], tmp[1], tmp[2], color=color_dic[label], alpha=0.5)
#         ax.set_xlim(-10, 40)
#         ax.set_ylim(-20, 50)
#         ax.set_zlim(-20, 60)
#         ax.view_init(30, angle)
#         plt.close()
#         fig.savefig(f"./Clustering/PCA3D_pictures/{name}/angle_{angle}.png")


images = []
for angle in tqdm.tqdm(range(360)):
    fig = plt.figure(figsize = (32, 32))
    ax = fig.add_subplot(projection='3d')
    for i in df['cluster'].unique():
        tmp = pca_df.loc[pca_df['cluster'] == i]
    ...
    ax.view_init(30, angle)
    plt.close()
    fig.savefig(f"./Clustering/PCA3D_pictures/all/angle_{angle}.png")